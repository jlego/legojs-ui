        ...
        value: function render() {
            ...
            function itemNav(item) {
                if (item.divider) {
                    ...
                    if (!item.children) {
                        return hx(_templateObject2$1, item.key, item.disabled ? "disabled" : "", item.href ? item.href : "javascript:;", item.title);
                    } else {
                        return loopNav(item);
                    }
                }
...
var Search = function(_Lego$UI$Baseview) {
    ...
    _createClass(Search, [ {
        ...
        value: function render() {
            var options = this.options || {};
            var vDom = hx(_templateObject, options.hasSelect ? hx(_templateObject2, options.vid, options.selectedTitle || "请选择", options.vid) : "", options.placeholder);
            return vDom;
        }
    }, {
        key: "clickSearch",
