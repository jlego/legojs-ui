...
var _templateObject3$1 = _taggedTemplateLiteral$1([ '<dropdown id="autocomplete_', '"></dropdown>' ], [ '<dropdown id="autocomplete_', '"></dropdown>' ]);
...
var Search = function(_Lego$UI$Baseview) {
    _inherits$1(Search, _Lego$UI$Baseview);
    function Search() {
        ...
        return _possibleConstructorReturn$1(this, (Search.__proto__ || Object.getPrototypeOf(Search)).call(this, options));
    }
    _createClass$1(Search, [ {
        ...
        value: function render() {
            ...
            var vDom = hx(_templateObject$1, opts.size ? "input-group-" + opts.size : "", opts.autoComplete ? "position: relative;" : "", opts.showSelect ? hx(_templateObject2$1, opts.vid, opts.activeValue || "请选择", opts.vid) : "", opts.vid, opts.placeholder, opts.name, val(opts.keyword), opts.autoComplete ? hx(_templateObject3$1, opts.vid) : "");
            return vDom;
        }
    }, {
        key: "_enterSearch",
        ...
        value: function render() {
            ...
            function itemNav(item) {
                if (item.divider) {
                    ...
                    if (!item.children) {
                        return hx(_templateObject2, item.isHidden ? 'style="display:none;"' : "", val(item.key), item.disabled || item.selected ? "disabled" : "", item.active ? "active" : "", item.href ? item.href : "javascript:;", val(item.value, item));
                    } else {
                        return loopNav(item);
                    }
                }
